...
/**
 * SOME COMMENTS GENERATED WITH THE HELP OF CLAUDE.AI
 *
 * Business logic for the charts view's two graphs:
 * the Monday-Sunday trend line overlay and the average
 * spending bar graph.
 *
 * All  methods are stateless, they reveive transaction data
 * from the session layer and return derived display-ready values.
...
public class ChartCalculator {
     ...
     * @return map of category name to average amount per transaction,
     ...
     * Returns one value per day of the week (Mon-Sun).
     *
     * When {@code currentWeekOnly} is {@code true}, only this week's
     * transactions are included. Used for the blue "current week"
     * overlay line.
     * When {@code false}, all transactions are included. Used for grey
     * "total average" reference line.
     *
     * @param transactions      the full in-memory transaction list
     * @param currentWeekOnly   true to restrict to the current week only
     * @return map of day of week to average spending on that day,
    ...
    public Map<DayOfWeek, Double> weekOverlayData(List<Transaction> transactions, boolean currentWeekOnly) {
        ...
        // avg each day's amounts; days with no transactions produce 0.0
     ...
     * for hover tooltips rounded to 2 decimal places.
     *
